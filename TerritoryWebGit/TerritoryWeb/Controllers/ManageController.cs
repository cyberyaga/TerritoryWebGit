    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ...
            var model = new IndexViewModel
            {
                ...
                UserRoles = string.Join(", ", await UserManager.GetRolesAsync(userId)),
        ...
        public async Task<ActionResult> Index(IndexViewModel userInfo)
        {
            ...
            {
                AspNetUser u = db.AspNetUsers.Find(userInfo.UserID);

                //If username is changed, then check unique
                if (u.UserName != userInfo.UserName)
                {
                    //if another person has this ID, stop everything
                    if (db.AspNetUsers.Any(x => x.Id != userInfo.UserID && x.UserName == userInfo.UserName))
                    {
                        userInfo.HasPassword = HasPassword();
                        userInfo.UserRoles = string.Join(", ", await UserManager.GetRolesAsync(userInfo.UserID));
                ...
                u.CellPhone = userInfo.CellPhone;
                u.FirstName = userInfo.FirstName;
                u.LastName = userInfo.LastName;
                u.Address1 = userInfo.Address1;
                u.Address2 = userInfo.Address2;
